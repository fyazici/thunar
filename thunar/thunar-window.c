...
static void thunar_hack_update_view_settings(ThunarWindow *window)
{
    ...
    GtkTreeSortable *sortable = GTK_TREE_SORTABLE(standard_view->model);
    gint last_sort_column, last_sort_order;
    GError *err=NULL;
    ...
    do
    {
        ...
        if (g_file_query_exists(test_file, NULL)) {
            ...
            if (indstream != NULL) {
                ...
                last_sort_column = g_ascii_strtoll(line, &end, 10);
                if (end == NULL) break;
                
                last_sort_order = g_ascii_strtoll(end, NULL, 10);
            }
        } else {
            /* no, just load the default settings from preferences */
            g_object_get(G_OBJECT(window->preferences),
                         "last-sort-column", &last_sort_column,
                         "last-sort-order", &last_sort_order,
                         NULL);
        }
        
        /* update the current view model without notifying xfconf setter */
        thunar_hack_list_model_set_sort_column_id_no_notify(sortable, last_sort_column, last_sort_order);
